...
public static class FrameworkRedirect
{
	...
	private static readonly Dictionary<string, FrameworkRedirectInfo> frmRedir2;

	private static readonly Dictionary<string, FrameworkRedirectInfo> frmRedir4;
	...
	private static bool TryApplyFrameworkRedirect(IAssembly assembly, Dictionary<string, FrameworkRedirectInfo> frmRedir, out IAssembly redirectedAssembly)
	{
		redirectedAssembly = null;
		...
		if (!frmRedir.TryGetValue(assembly.Name, out var value))
		{
			return false;
		}
		if (PublicKeyBase.TokenCompareTo(assembly.PublicKeyOrToken, value.publicKeyToken) != 0)
		{
			return false;
		}
		if (Utils.CompareTo(assembly.Version, value.redirectVersion) == 0)
		{
		...
		else
		{
			...
			redirectedAssembly.Version = value.redirectVersion;
		}
		return true;
	}
}
