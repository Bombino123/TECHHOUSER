...
public sealed class SQLiteIndex
{
	...
	internal static void FromIntPtr(IntPtr pIndex, bool includeOutput, ref SQLiteIndex index)
	{
		...
		for (int i = 0; i < num; i++)
		{
			UnsafeNativeMethods.sqlite3_index_constraint constraint = (UnsafeNativeMethods.sqlite3_index_constraint)Marshal.PtrToStructure(SQLiteMarshal.IntPtrForOffset(pointer, i * num3), typeFromHandle);
		...
		{
			UnsafeNativeMethods.sqlite3_index_orderby orderBy = (UnsafeNativeMethods.sqlite3_index_orderby)Marshal.PtrToStructure(SQLiteMarshal.IntPtrForOffset(pointer2, j * num4), typeFromHandle2);
		...
		if (constraintUsages != null)
		{
			...
			for (int k = 0; k < num; k++)
			{
				UnsafeNativeMethods.sqlite3_index_constraint_usage constraintUsage = (UnsafeNativeMethods.sqlite3_index_constraint_usage)Marshal.PtrToStructure(SQLiteMarshal.IntPtrForOffset(pointer3, k * num5), typeFromHandle3);
	...
	internal static void ToIntPtr(SQLiteIndex index, IntPtr pIndex, bool includeInput)
	{
		...
		if (includeInput)
		{
			...
			for (int i = 0; i < num; i++)
			{
				Marshal.StructureToPtr(new UnsafeNativeMethods.sqlite3_index_constraint(array[i]), SQLiteMarshal.IntPtrForOffset(pointer, i * num2), fDeleteOld: false);
			}
		}
		offset = SQLiteMarshal.NextOffsetOf(offset, IntPtr.Size, 4);
		...
		if (includeInput)
		{
			...
			{
				Marshal.StructureToPtr(new UnsafeNativeMethods.sqlite3_index_orderby(array2[j]), SQLiteMarshal.IntPtrForOffset(pointer2, j * num4), fDeleteOld: false);
			}
		}
		offset = SQLiteMarshal.NextOffsetOf(offset, IntPtr.Size, IntPtr.Size);
		...
		for (int k = 0; k < num; k++)
		{
			Marshal.StructureToPtr(new UnsafeNativeMethods.sqlite3_index_constraint_usage(constraintUsages[k]), SQLiteMarshal.IntPtrForOffset(pointer3, k * num5), fDeleteOld: false);
		}
		offset = SQLiteMarshal.NextOffsetOf(offset, IntPtr.Size, 4);
		SQLiteMarshal.WriteInt32(pIndex, offset, sQLiteIndexOutputs.IndexNumber);
		offset = SQLiteMarshal.NextOffsetOf(offset, 4, IntPtr.Size);
		SQLiteMarshal.WriteIntPtr(pIndex, offset, SQLiteString.Utf8IntPtrFromString(sQLiteIndexOutputs.IndexString, tracked: false));
		offset = SQLiteMarshal.NextOffsetOf(offset, IntPtr.Size, 4);
