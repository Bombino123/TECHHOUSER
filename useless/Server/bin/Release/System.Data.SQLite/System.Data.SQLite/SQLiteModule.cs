...
public abstract class SQLiteModule : ISQLiteManagedModule, IDisposable
{
	...
	private SQLiteErrorCode DestroyOrDisconnect(bool destroy, IntPtr pVtab)
	{
		...
		catch (Exception ex)
		{
			try
			{
				...
				{
					SQLiteLog.LogMessage(-2146233088, HelperMethods.StringFormat(CultureInfo.CurrentCulture, "Caught exception in \"{0}\" method: {1}", destroy ? "xDestroy" : "xDisconnect", ex));
				}
			}
			catch
			{
	...
	private static bool SetTableError(SQLiteModule module, IntPtr pVtab, bool logErrors, bool logExceptions, string error)
	{
		try
		{
			...
			{
				SQLiteLog.LogMessage(SQLiteErrorCode.Error, HelperMethods.StringFormat(CultureInfo.CurrentCulture, "Virtual table error: {0}", error));
			}
		}
		catch
		{
		...
		catch (Exception ex)
		{
			try
			{
				if (logExceptions)
				{
					SQLiteLog.LogMessage(-2146233088, HelperMethods.StringFormat(CultureInfo.CurrentCulture, "Caught exception in \"{0}\" method: {1}", "SetTableError", ex));
				}
			}
			catch
			{
	...
	protected virtual SQLiteVirtualTable TableFromIntPtr(IntPtr pVtab)
	{
		...
		SetTableError(pVtab, HelperMethods.StringFormat(CultureInfo.CurrentCulture, "managed table for {0} not found", pVtab));
		return null;
	}
	...
	protected virtual SQLiteVirtualTableCursor CursorFromIntPtr(IntPtr pVtab, IntPtr pCursor)
	{
		...
		SetTableError(pVtab, HelperMethods.StringFormat(CultureInfo.CurrentCulture, "managed cursor for {0} not found", pCursor));
		return null;
	}
	...
	{
		return HelperMethods.StringFormat(CultureInfo.InvariantCulture, "{0}:{1}", argumentCount, name);
	}
	...
	protected virtual void Dispose(bool disposing)
	{
		...
		catch (Exception ex)
		{
			try
			{
				if (LogExceptionsNoThrow)
				{
					SQLiteLog.LogMessage(-2146233088, HelperMethods.StringFormat(CultureInfo.CurrentCulture, "Caught exception in \"{0}\" method: {1}", "Dispose", ex));
				}
			}
			catch
			{
