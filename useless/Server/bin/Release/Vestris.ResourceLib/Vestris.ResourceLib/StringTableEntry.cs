...
public class StringTableEntry
{
	...
	internal void Read(IntPtr lpRes)
	{
		...
		IntPtr ptr = new IntPtr(lpRes.ToInt64() + Marshal.SizeOf((object)_header));
		_key = Marshal.PtrToStringUni(ptr);
		IntPtr ptr2 = ResourceUtil.Align(ptr.ToInt64() + (_key.Length + 1) * Marshal.SystemDefaultCharSize);
		if (_header.wValueLength > 0)
		{
			_value = Marshal.PtrToStringUni(ptr2, _header.wValueLength);
	...
	internal void Write(BinaryWriter w)
	{
		...
		ResourceUtil.WriteAt(w, (w.BaseStream.Position - position2) / Marshal.SystemDefaultCharSize, position + 2);
