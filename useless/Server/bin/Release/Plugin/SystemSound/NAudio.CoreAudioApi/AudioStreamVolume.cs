...
public class AudioStreamVolume : IDisposable
{
	...
	public void SetAllVolumes(float[] levels)
	{
		...
		{
			throw new ArgumentOutOfRangeException("levels", string.Format(CultureInfo.InvariantCulture, "SetAllVolumes MUST be supplied with a volume level for ALL channels. The AudioStream has {0} channels and you supplied {1} channels.", new object[2] { channelCount, levels.Length }));
		}
