...
public static class IconInjector
{
	...
	private class IconFile
	{
		...
		public static IconFile FromByte(byte[] fileBytes)
		{
			IconFile iconFile = new IconFile();
			GCHandle gCHandle = GCHandle.Alloc(fileBytes, GCHandleType.Pinned);
			...
			for (int i = 0; i <= iconFile.iconDir.Count - 1; i++)
			{
				...
				Buffer.BlockCopy(fileBytes, iCONDIRENTRY.ImageOffset, iconFile.iconImage[i], 0, iCONDIRENTRY.BytesInRes);
				num += num2;
			}
			gCHandle.Free();
			return iconFile;
		...
		public byte[] CreateIconGroupData(uint iconBaseID)
		{
			...
			for (int i = 0; i <= ImageCount - 1; i++)
			{
				...
				gRPICONDIRENTRY.ID = Convert.ToUInt16(iconBaseID + i);
