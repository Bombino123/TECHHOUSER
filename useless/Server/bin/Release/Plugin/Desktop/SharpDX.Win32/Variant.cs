...
public struct Variant
{
	...
	public unsafe object Value
	{
		get
		{
			...
			case VariantType.Vector:
			{
				...
				case VariantElementType.StringPointer:
				{
					...
					for (int l = 0; l < num; l++)
					{
						array11[l] = Marshal.PtrToStringAnsi(*(IntPtr*)((byte*)(void*)variantValue.recordValue.RecordPointer + (nint)l * (nint)sizeof(IntPtr)));
					}
					return array11;
				}
				case VariantElementType.WStringPointer:
				{
					...
					for (int k = 0; k < num; k++)
					{
						array8[k] = Marshal.PtrToStringUni(*(IntPtr*)((byte*)(void*)variantValue.recordValue.RecordPointer + (nint)k * (nint)sizeof(IntPtr)));
					}
					return array8;
				}
				case VariantElementType.Dispatch:
				case VariantElementType.ComUnknown:
				{
					...
					for (int j = 0; j < num; j++)
					{
						array5[j] = new ComObject(*(IntPtr*)((byte*)(void*)variantValue.recordValue.RecordPointer + (nint)j * (nint)sizeof(IntPtr)));
					}
					return array5;
				}
				case VariantElementType.Pointer:
				...
				case VariantElementType.FileTime:
				{
					...
					for (int i = 0; i < num; i++)
					{
						array[i] = DateTime.FromFileTime(*(long*)((byte*)(void*)variantValue.recordValue.RecordPointer + (nint)i * (nint)8));
					}
					return array;
				}
				default:
		...
		set
		{
			...
			Type = VariantType.Default;
			if (SharpDXTypeExtensions.GetTypeInfo(type).IsPrimitive)
			{
				if (type == typeof(byte))
				{
